...
ttk.Button(text="打开", width=10, command=open_file_or_dir, bootstyle="primary").place(x=400, y=170)
...
def pdf2pngs(pdf_path):
    ...
    for index, image in enumerate(images):
        image_path = '{}\{}.png'.format(dst_dir, index)
        image.save(image_path)
        # 平滑处理图像以减少锯齿状边缘
        smoothed_image = Image.open(image_path)
        smoothed_image = smoothed_image.filter(ImageFilter.SMOOTH_MORE)
        smoothed_image.save(image_path, "PNG")


def merge_png2pdf(path):
    # 以防同名文件夹被误删
    png_path = path + '-tmp'
    png_files = [join(png_path, fn) for fn in listdir(png_path) if fn.endswith('.png')]
    png_files.sort(key=lambda fn: int(splitext(basename(fn))[0]))
    images = []
    for png_file in png_files:
        image = Image.open(png_file)
        images.append(image.convert("RGB"))

    images[0].save(path + '.pdf', save_all=True, append_images=images[1:], optimize=False)
    # 以防同名文件夹被误删
    for pic in png_files:
        remove(pic)
    if not len(os.listdir(png_path)):
...
def get_doc(path):
    ...
    try:
        ...
        return False
    pdf2pngs(pdf_file)
    merge_png2pdf(splitext(pdf_file)[0])
    return True
